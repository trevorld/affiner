...
test_that("project2d()", {
    x <- c(2, 5, 7)
    y <- c(3, 4, 6)
    p1 <- as_coord2d(x = x, y = y)$project(0)
    expect_equal(p1$x, x)
    expect_equal(p1$y, rep(0, 3))

    expect_equal(as.double(as_coord2d(x, y), 0), p1$x)

    p2 <- as_coord2d(x = x, y = y)$project(90, "degrees")
    expect_equal(p2$x, rep(0, 3))
    expect_equal(p2$y, y)

    p3 <- as_coord2d(x = x, y = y)$project(0, scale = 0.5)
...
test_that("reflect2d()", {
    ...
    expect_equal(p2$x, -x)
    expect_equal(p2$y, y)
    p3 <- as_coord2d(x = x, y = y)$reflect(135, "degrees")
    expect_equal(p3$x, -y)
    expect_equal(p3$y, -x)
