...
img_filename <- function(suit, rank) {
    stopifnot(suit %in% c("Clubs", "Diamonds", "Hearts", "Spades"),
              rank %in% c("Jack", "King"))
...
grob_left <- affineGrob(rasterGrob(img_j), vp_define = vp_define,
                        transform = left_settings$transform, vp_use = left_settings$vp)
grob_right <- affineGrob(rasterGrob(img_k), vp_define = vp_define,
                         transform = right_settings$transform, vp_use = right_settings$vp)
grob_text <- grobTree(polygonGrob(x=c(0, 0, 1, 1), y = c(1, 0, 0, 1), gp = gpar(fill = grey, col = NA)),
                      textGrob("affiner", x = 0.42, y = 0.6, rot = 45,
                               gp = gpar(fontfamily = "Pinyon Script", fontsize = 90)))
grob_top <- affineGrob(grob_text, vp_define = vp_define,
                       transform = top_settings$transform, vp_use = top_settings$vp)
