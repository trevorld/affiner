...
vignette: >
  %\VignetteIndexEntry{Introduction to affiner}
...
```{r hex-logo, fig.width = 4.5, fig.height = 4.5, fig.alt = "Isometric-cube hex logo"}
library("affiner")
library("grid")
...
```{r hex-logo2, fig.width = 4.5, fig.height = 4.5, fig.alt = "Isometric-cube hex logo"}
grid.newpage()
...
```{r die-faces, fig.width = 4.0, fig.height = 3.0, fig.alt = "The six die faces"}
library("affiner")
library("grid")
...
```
```{r projected-die, fig.width = 3.0, fig.height = 3.0, fig.alt = "Parallel projection of a die"}
