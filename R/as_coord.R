...
as_coord3d.angle <- function(x, radius = 1, inclination = NULL, z = NULL, ...) {
	stopifnot(!(is.null(inclination) && is.null(z)), is.null(inclination) || is.null(z))
	...
	if (is.null(inclination)) {
		# cylindrical coordinates
		as_coord3d(as_coord2d(x, radius = radius), z = z)
	} else {
		# spherical coordinates
		as_coord3d(
			x = radius * sin(inclination) * cos(x),
...
as_coord2d_character_x <- function(x) {
	switch(x, "origin" = 0, "x-axis" = 1, "y-axis" = 0, NA_real_)
}

as_coord2d_character_y <- function(x) {
	switch(x, "origin" = 0, "x-axis" = 0, "y-axis" = 1, NA_real_)
}
...
as_coord3d_character_x <- function(x) {
	switch(x, "origin" = 0, "x-axis" = 1, "y-axis" = 0, "z-axis" = 0, NA_real_)
}

as_coord3d_character_y <- function(x) {
	switch(x, "origin" = 0, "x-axis" = 0, "y-axis" = 1, "z-axis" = 0, NA_real_)
}

as_coord3d_character_z <- function(x) {
	switch(x, "origin" = 0, "x-axis" = 0, "y-axis" = 0, "z-axis" = 1, NA_real_)
}
...
as_coord1d.Coord2D <- function(
	...
	theta <- as_angle(line)
	xs <- x$clone()$permute(permutation)$translate(-closest)$rotate(-theta)$shear(
		xy_shear = scale
	)$x
...
as_coord3d.data.frame <- function(x, ..., z = NULL) {
	stopifnot(all(hasName(x, c("x", "y"))), is.null(z) || !hasName(x, "z"))
	if (!is.null(z)) {
		x$z <- z
	}
	if (hasName(x, "z")) {
...
as_xw_matrix <- function(x) {
	stopifnot(ncol(x) == 1L || ncol(x) == 2L, is.numeric(x))
	if (ncol(x) < 2L) {
		x <- cbind(x, 1)
	} else {
...
as_xyw_matrix <- function(x) {
	...
	stopifnot(ncol(x) == 2L || ncol(x) == 3L, is.numeric(x))
	if (ncol(x) < 3L) {
		x <- cbind(x, 1)
	} else {
...
as_xyzw_matrix <- function(x) {
	...
	stopifnot(ncol(x) >= 2L, ncol(x) <= 4L, is.numeric(x))
	if (ncol(x) == 2L) {
		x <- cbind(x, 0, 1)
	} else if (ncol(x) == 3L) {
		x <- cbind(x, 1)
