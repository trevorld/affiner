...
standardize.Plane3D <- function(x) {
    a <- ifelse(is_congruent(x$c, 0),
                ifelse(is_congruent(x$b, 0), 1, x$a / x$b),
                x$a / x$c)
    b <- ifelse(is_congruent(x$c, 0),
                ifelse(is_congruent(x$b, 0), 0, 1),
                x$b / x$c)
    c <- ifelse(is_congruent(x$c, 0), 0, 1)
    d <- ifelse(is_congruent(x$c, 0),
                ifelse(is_congruent(x$b, 0), x$d / a, x$d / b),
                x$d / x$c)
    as_plane3d(a, b, c, d)
}
