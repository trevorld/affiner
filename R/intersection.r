...
#' @rdname intersection
#' @export
intersection.Point1D <- function(x, y, ...,
...
#' @export
intersection.Line2D <- function(x, y, ...,
                                tolerance = sqrt(.Machine$double.eps)) {
    if (is_coord2d(y))
    ...
    for (i in seq_len(n)) {
        ...
        if (is_equivalent(xi, yi, tolerance = tolerance)) {
            ...
            m <- matrix(c(xi$a, xi$b, yi$a, yi$b),
...
intersection_Coord2D_Line2D <- function(x, y, ...,
                                       tolerance = sqrt(.Machine$double.eps)) {
    stopifnot(!any(is_degenerate(x)),
              !any(is_degenerate(y)))
    n <- max(length(x), length(y))
    ...
    for (i in seq_len(n)) {
        xi <- x[i] # Coord2D
        yi <- y[i] # Line2D
        if (is_equivalent(yi$a * xi$x + yi$b * xi$y, -yi$c,
                          tolerance = tolerance)) {
            l[[i]] <- xi
        } # else already NULL
    }
...
#' @rdname intersection
#' @export
intersection.Plane3D <- function(x, y, ...,
                                 tolerance = sqrt(.Machine$double.eps)) {
    if (is_coord3d(y)) {
...
intersection_Coord3D_Plane3D <- function(x, y, ...,
                                         tolerance = sqrt(.Machine$double.eps)) {
    stopifnot(!any(is_degenerate(x)),
              !any(is_degenerate(y)))
    n <- max(length(x), length(y))
    ...
    for (i in seq_len(n)) {
        xi <- x[i] # Coord3D
        yi <- y[i] # Plane3D
        if (is_equivalent(yi$a * xi$x + yi$b * xi$y + yi$c * xi$z, -yi$d,
                          tolerance = tolerance)) {
            l[[i]] <- xi
        } # else already NULL
    }
...
#' @rdname intersection
#' @export
intersection.Coord1D <- function(x, y, ...,
                                 tolerance = sqrt(.Machine$double.eps)) {
    if (!is_coord1d(y)) {
        y <- as_coord1d(y)
    }
...
#' @rdname intersection
#' @export
intersection.Coord2D <- function(x, y, ...,
                                 tolerance = sqrt(.Machine$double.eps)) {
    if (is_line2d(y))
...
#' @rdname intersection
#' @export
intersection.Coord3D <- function(x, y, ...,
                                 tolerance = sqrt(.Machine$double.eps)) {
    if (is_plane3d(y)) {
